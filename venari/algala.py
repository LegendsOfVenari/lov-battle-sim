...
class Algala(Venari):
    ...
    def use_ability(self, target):
        ...
        print(f"{self.name} charged recklessly at {target.name}, dealing {damage:.2f} damage!")
        ...
        if not has_armor:
            ...
            print(f"{self.name} took {damage:.2f} damage from its own charge!")
        else:
            # Remove one Armor stack from Algala
            for effect in self.active_effects:
                if isinstance(effect, Armor):
                    effect.on_damage_received(self, damage)
                    print(f"{self.name}'s Armor stack protected it from self-inflicted damage!")
                    break
    ...
    def on_swap_in(self, enemy_team=None):
        ...
        if boost_amount > 0:
            self.apply_effect(AttackDamageBuff(boost_amount))
            print(f"{self.name} gains {boost_amount} AD due to its missing health!")
