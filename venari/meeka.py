...
class Meeka(Venari):
    def basic_attack(self, target):
        ...
        if self.battle_handler.has_effect(effect.get_id()):
            self.battle_handler.remove_effect(effect.get_id())
    ...
    def use_ability(self, target):
        ...
        if self.target.battle_stats.hp_percentage() >= 0.75:
            self.deal_damage(target, 110, DamageType.AD)
        else:
            self.deal_damage(target, 110, DamageType.AD)
    ...
    def get_opportunist_effect(self):
        return Unique(self.messages, "opportunist", 5, 5)
